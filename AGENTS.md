...
- **`embeddings.json` + `milvus.db` are COMMITTED build artifacts, not git-ignored.** The vector index is regenerated **only when the chunks change** — by the weekly `refresh.yml`, or by a PR that edits `chunks.json`/chunking config — via `scripts/ensure-index.sh`, and committed. Releases then pack the committed `milvus.db` straight from the repo; a chat-UI-only release does **no** embedding. `ci.yml` runs `scripts/ensure-index.sh --check` (a canonical-hash comparison — no model download) and fails any PR that changed `chunks.json` without regenerating the index. So if you touch chunks/config locally, run `./scripts/ensure-index.sh` and commit `.opencrane/embeddings.json` + `.opencrane/milvus.db` alongside. (`chunks.json` + `llmstxt/` remain committed inputs. Don't reintroduce `opencrane embed` into the release path — that's the every-release re-embed this replaced; opencrane's own up-to-date check is broken, which is why `ensure-index.sh` does the comparison itself.)
