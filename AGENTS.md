...
- **opencrane is version-locked in `.opencrane/opencrane-version`.** CI, `refresh.yml`, `release.yml`, and `scripts/ensure-index.sh` all invoke `uvx opencrane@<that version>` — never a floating `latest`, so the index and the packaged wheel are reproducible. To upgrade, edit that one file and regenerate the index (`./scripts/ensure-index.sh`) in the same PR; never revert a `uvx opencrane` call to an unpinned form.
