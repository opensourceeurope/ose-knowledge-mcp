...
- **`embeddings.json` + `milvus.db` are COMMITTED build artifacts, not git-ignored.** The vector index is regenerated **only when the chunks change** — by the weekly `refresh.yml`, or by a PR that edits `chunks.json`/chunking config — via `scripts/ensure-index.sh`, and committed. (A manual `refresh.yml` run with `force_reindex` is the one exception: it drops the artifacts first to force a rebuild on the pinned opencrane, e.g. after an opencrane bump.) Releases then pack the committed `milvus.db` straight from the repo; a chat-UI-only release does **no** embedding. `ci.yml` runs `scripts/ensure-index.sh --check` (a canonical-hash comparison — no model download) and fails any PR that changed `chunks.json` without regenerating the index. So if you touch chunks/config locally, run `./scripts/ensure-index.sh` and commit `.opencrane/embeddings.json` + `.opencrane/milvus.db` alongside. (`chunks.json` + `llmstxt/` remain committed inputs. Don't reintroduce `opencrane embed` into the release path — that's the every-release re-embed this replaced; opencrane's own up-to-date check is broken, which is why `ensure-index.sh` does the comparison itself.)
- **opencrane is version-locked in `.opencrane/opencrane-version`.** CI, `refresh.yml`, `release.yml`, and `scripts/ensure-index.sh` all invoke `uvx opencrane@<that version>` — never a floating `latest`, so the index and the packaged wheel are reproducible. To upgrade, edit that one file and regenerate the index (`./scripts/ensure-index.sh`) in the same PR; never revert a `uvx opencrane` call to an unpinned form. **Build-pipeline steps (`fetch`/`chunk`, and the `build` shorthand) must request the `[pipeline]` extra — `uvx opencrane[pipeline]@<version>` — because docling/PyGithub/tiktoken live in opencrane's optional `[pipeline]` extra, kept out of the serve-only base so the packed `ose-knowledge-mcp` wheel installs slim (fast `uvx` cold start). `embed`/`index`/`serve`/`pack` run on the base — plain `uvx opencrane@<version>`.**
